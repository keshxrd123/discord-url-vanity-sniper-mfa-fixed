...
func main() {
	...
	setTitle("keshxrd")
	for i := 0; i < 9; i++ {
		logInfo("Configs", "keshxrd")
	}

	cfg, err := loadConfig("keshxrd.json")
	if err != nil {
    ...
    if config.Token == "" || config.Password == "" || config.GuildID == "" || config.WebhookURL == "" || config.SelfToken == "" {
        logError("Configs", "Config values cannot be empty!")
        os.Exit(1)
    }
	...
	if err != nil {
		...
		if err := sendFailureWebhook(config.NewVanityURL, 0); err != nil { 
			logError("Checker", fmt.Sprintf("Failed to send failure webhook: %v", err))
		}
		os.Exit(1)
	}
